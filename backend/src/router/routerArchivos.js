...
router.get('/Archivos', async (req, res) => {
    try {
      ...
      console.error('Error al obtener las archivos:', error);
      res.status(500).json({
        status: 500,
...
router.post('/archivos', upload.single('archivo'), async (req, res) => {
  try {
    ...
    const proyecto = await Proyecto.findOne({ where: { id_proyecto: proyectoId } });
    if (!proyecto) {
    ...
    // Guarda la ruta del archivo y la fecha en la base de datos
...
//borrar archivo
router.delete('/archivos/:id_proyecto', async (req, res) => {
  try {
    const { id_proyecto } = req.params;

    // Utiliza await para esperar la resolución de la promesa
    const resultado = await Archivos.findOne({
      where: {
    ...
    if (!resultado) {
      return res.status(404).json({ mensaje: 'No se encontraron archivos para el proyecto' });
    }

    // Realiza la eliminación utilizando el método destroy
    ...
    res.status(200).json({ mensaje: 'Archivos eliminados correctamente' });
  } catch (error) {
    console.error(error);
...
router.get('/archivos/proyecto/mostrar/:proyectoId', async (req, res) => {
  try {
    ...
    if (!archivo) {
      return res.status(404).json({ error: 'Archivo no encontrado para el proyecto especificado' });
    }
